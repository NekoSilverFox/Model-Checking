...
Q \Rightarrow R
$$
这意味着满足 $Q$ 的所有情况，必然也满足 $R$。

**特别说明：** 

- **True**：最**弱**的条件，它总是成立 (самое слабое условие (слабейший предикат))
...
SP 用于描述程序运行后的最终状态以及如何从程序的输入状态推导出最终状态。它的主要用途包括：

- **程序验证**：通过前置条件和最强后置条件，证明程序是否达到了预期的结果。
- **程序分析**：通过推导最强后置条件来理解程序可能的所有输出状态
